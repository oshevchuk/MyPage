/**
 * Created by Oshevchuk on 06.07.2017.
 */
...
(function() {
    ...
    function moveIt()
    {
        for(i = 0; i < circs.length; ++i)
        {
            ...
            nowY += dy;
            // Calc wrap around
            if (nowX < 0) nowX = window.innerWidth + nowX;
            else          nowX = nowX % innerWidth;
            if (nowY < 0) nowY = window.innerHeight + nowY;
            else          nowY = nowY % innerHeight;
    ...
    window.onload = function () {
        ...
        {
            opa = ran(1,9)/10;
            circs.push(paper.circle(ran(0,window.innerWidth), ran(0,window.innerHeight), ran(1,30)).attr({"fill-opacity": opa,
                "stroke-opacity": opa}));
        }
